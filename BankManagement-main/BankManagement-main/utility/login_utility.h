...
bool login_handler(bool is_admin, int connection_descriptor, struct User *user_id_ptr) {
    ...
    if(is_admin){
        ...
        strtok(temp_buffer,"-");//to extract user id//user-123 becomes user/0123
        char* token = strtok(NULL,"-");//When the first argument is NULL, strtok() continues tokenizing 
        //the same string from where it left off in the previous call.will search for next - from 123.but there is no therefore return null.
        //Keep in mind that strtok() is a destructive function, as it modifies the input string by placing null terminators to separate tokens. If you need to preserve the original temp_buffer, you may need to create a copy before using strtok().
        if(token == NULL) {
